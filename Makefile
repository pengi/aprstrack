...
CFLAGS += $(CSTANDARD)
CFLAGS += -DF_CPU=$(F_CPU)
#CFLAGS += -DSERIAL_BAUDRATE=$(SERIAL_BAUDRATE)
CFLAGS += -DSERIAL_BAUDRATE=9600
CFLAGS += -DTRACKERID=$(TRACKERID)
...
LDFLAGS += $(EXTMEMOPTS)
LDFLAGS += $(EXTRA_LIBS) $(PRINTF_LIB) $(SCANF_LIB) $(MATH_LIB)
...
AVRDUDE_FLAGS += -U lfuse:w:$(LOWFUSE):m
AVRDUDE_FLAGS += -U hfuse:w:$(HIGHFUSE):m
AVRDUDE_FLAGS += -U efuse:w:$(EXTFUSE):m
